...
exports.updateJob = async (req, res, next) => {
    try {
        const job = await Job.findByIdAndUpdate(req.params.job_id, req.body, {new: true}).populate('jobType', 'jopTypeName').populate('user', 'firstName'); //new: true returns the updated job
        res.status(200).json({
            success: true,
            job
        });
...
exports.showJobs = async (req, res, next) => {
    ...
    try {
        const jobs = await Job.find({...keyword, jobType : categ, location : locationFilter}).sort({ createdAt : -1}).skip(pageSize * (page - 1)).limit(pageSize); //skip() skips the first 5 jobs
        res.status(200).json({
            success: true,
            jobs,
            page,
