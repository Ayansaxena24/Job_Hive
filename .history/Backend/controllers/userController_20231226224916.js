...
exports.changeStatus = async (req, res, next) => {
    ...
    try {
        ...
        const {jobHistoryId, userId, newStatus} = req.body;
        const userData = await User.findOne({_id: userId});
        const jobHistoryArray = userData.jobHistory;

         // Find the index of the jobHistory object with the specified jobHistoryId
         const jobHistoryIndex = jobHistoryArray.findIndex(
            (job) => job._id.toString() === jobHistoryId
        );

         // If the jobHistory object is found
         if (jobHistoryIndex !== -1) {
            // Update the applicationStatus of the specified jobHistory object
            jobHistoryArray[jobHistoryIndex].applicationStatus = newStatus;
            ...
            res.status(200).json({
                ...
                updatedJobHistory: jobHistoryArray[jobHistoryIndex],
            });
        } else {
            res.status(404).json({
                success: false,
