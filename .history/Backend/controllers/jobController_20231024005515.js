...
exports.showJobs = async (req, res, next) => {
    ...
    try {
        const jobs = await Job.find({...keyword, jobType : categ, location : locationFilter}).sort({ createdAt : -1}).populate('jobType', 'j').populate('user', 'firstName').skip(pageSize * (page - 1)).limit(pageSize)
        console.log(jobs);
        res.status(200).json({
            success: true,
            jobs,
