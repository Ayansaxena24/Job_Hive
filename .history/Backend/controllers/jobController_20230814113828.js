...
exports.showJobs = async (req, res, next) => {
    ...
    try {
        const jobs = await Job.find({...keyword, jobType : categ, location : locationFilter}).sort({ createdAt : }).skip(pageSize * (page - 1)).limit(pageSize); //skip() skips the first 5 jobs
        res.status(200).json({
            success: true,
            jobs,
            page,
