...
const SingleJob = () => {
    ...
    return (
        <div className={`${mode === 'light' ? ' min-h-screen' : 'bg-gradient-to-t from-gray-700 via-gray-900 to-black border-1'}`}>
            <Box
            className={`${mode === 'light' ? ' min-h-screen min-w-screen' : 'bg-gradient-to-t from-gray-700 via-gray-900 to-black border-1'}`}
            // sx={{ bgcolor: "#fafafa", height: 50 }}
            >

                <Navbar />
                {/* <img src={greenSpeaker} alt="green speaker" className="w-[10%] absolute" /> */}
                <Box className="absolute top-12 left-0 w-full bg-green-900 h-screen">
                    <Container className="flex justify-center items-center" sx={{ pt: '30px' }}>
                        ...
                        >
                            ...
                            <Box className="flex justify-center pr-12">
                                <Button onClick={applyForAJob} className='w-[1100px] h-20' sx={{ fontSize: "15px" }} variant='contained'>Apply for this Job</Button>
                            </Box>
