  ...
  const deleteUserById = (e,id) => {
    ...
    const columns = [
...

        {
            ...
            width: 150,
            renderCell: (params) => (
                params.row.role === 1 ? "Admin" : (params.row.role === 0 ? "Regular user" : "Job Creator")
            )
        },
...

        {
            ...
            renderCell: (values) => (
                <Box sx={{ display: "flex", justifyContent: "space-between", width: "170px" }}>
                    <Button variant="contained"><Link style={{ color: "white", textDecoration: "none" }} to={`/admin/user/user/${values.row._id}`}>Edit</Link></ Button>
