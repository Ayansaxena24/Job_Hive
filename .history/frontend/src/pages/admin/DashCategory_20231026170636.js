        ...
        },
        {
            ...
            renderCell: (values) => (
                <Box className = "flex justify-space-between">
                    <button variant='contained'><Link className='text-white px-2 py-1 hover:text-purple-500 rounded-xl border-1 bg-black' to={`/admin/edit/user/${values.row.id}`}>Edit</Link></button>
                    <button className="px-1 hover:text-purple-500 px-3 py-1 rounded-xl border-1 bg-black" onClick={(e) => deleteJobCategoryById(e, values.row.id)} variant='contained' color='error'>Delete</button>
                </Box>
            )
        },
    ];
