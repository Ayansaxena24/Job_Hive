        ...
        },
        {
            ...
            rederCell: (values) => (
                <Box className = "flex justify-space-between">
                    <button variant='contained'><Link className='text-white z-30' to={`/admin/edit/user/${values.row.id}`}>Edit</Link></button>
                    <button className='z-30 text-white' onClick={(e) => deleteJobCategoryById(e, values.row.id)} variant='contained' color='error'>Delete</button>
                </Box>
            )
        },
    ];
