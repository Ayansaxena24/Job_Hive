  ...
  const columns = [
...

    {
      ...
      renderCell: (values) => (
        ...
        >
          <button onClick={values.row.resumeLink ? window.open(values.row.resumeLink) : 
            // console.log("fine")
            ""
            } className="bg-gradient-to-t from-gray-700 via-gray-900 to-gray-700 rounded-2xl px-6 py-1 duration-300 ease-in-out hover:bg-gradient-to-b hover:from-gray-900 hover:via-gray-700 hover:to-gray-900" variant="contained">Resume</ button>
...

      {
        ...
        renderCell: (values) => (
          ...
          >
            ...
            <button
              onClick={() => handleStatusChange(values.row.userId, values.row._id)}
    ...
    const handleStatusChange = async (userId, jobHistoryId) => {
      try {
        if (!selectedStatus || !userId || !jobHistoryId) {
          console.error("Missing required parameters:", { selectedStatus, userId, jobHistoryId });
          return;
        }
    
        console.log("Before Axios request - userId:", userId, "jobHistoryId:", jobHistoryId);
    
        // Send Axios post request to update status
        const response = await axios.post("http://localhost:9000/api/creator/changeStatus", {
          userId,
          jobHistoryId,
