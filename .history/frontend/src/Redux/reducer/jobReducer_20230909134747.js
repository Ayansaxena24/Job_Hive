...
;
export const loadAdminJobReducer = (
  state = { jobs: [] },
  action //initial state is empty array
) => {
  switch (action.type) {
    case ADMIN_JOB_LOAD_REQUESTJOB_LOAD_REQUEST: //if request is made, loading is true
      return { loading: true };
    case ADMIN_JOB_LOAD_SUCCESS: //if request is successful, loading is false and jobs are returned
      return {
        loading: false,
        success: action.payload.success,
        page: action.payload.page,
    ...
    case ADMIN_JOB_LOAD_FAIL: //if request is successful, loading is false and jobs are returned
      return {
        loading: false,
        error: action.payload,
      };

    case ADMIN_JOB_LOAD_RESET: //if request is successful, loading is false and jobs are returned
      return {};
    default:
      return state;
  }
