...
export const jobLoadAction = (pageNumber, keyword = '', cat = '', location = '') => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get(`${backend_api}${backend_api}/api/jobs/show/?pageNumber=${pageNumber}&keyword=${keyword}&cat=${cat}&location=${location}`);
...
export const jobLoadSingleAction = (id) => async (dispatch) => {
    ...
    try {
        const { data } = await axios.get(`${backend_api}${backend_api}/api/job/${id}`);
        console.log(data);
        const {jobTypeName} = data.job.jobType;
        // console.log(data.job.jobType);
        const {_id, company, title, description, salary, location, user, createdAt, updatedAt, available} = data.job;
