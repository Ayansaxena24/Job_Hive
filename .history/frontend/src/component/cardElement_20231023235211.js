...
const CardElement = ({ jobTitle, description, category, location, id, isCreator }) => {
    ...
    return (
        ...
        <div className={`p-2 my-2 rounded-md ${mode === 'dark' ? 'bg-gradient-to-b from-gray-900 to-gray-600 bg-gradient-to-r hover:bg-gradient-to-tr hover:from-gray-900 hover:to-gray-600 hover:bg-gradient-to-r duration-300 ease-in-out' : 'bg-gray-200 text-black border-2 border-gray-100'}`} sx={{ minWidth: 275, mb: 3, mt: 3}}>
            ...
            <CardActions>
                <Button disableElevation variant='contained' size="small" startIcon={<AddIcon />}><Link style={{ textDecoration: "none", color: "white", boxShadow: 0 }} to={`${url}?salary=${}&category=${}&location=${}&desc=${desc}`}>More Details</Link></Button>
            </CardActions>
        </div>
        </div>
    );
